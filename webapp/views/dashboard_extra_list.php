  ...
  */
  function changeClickTracking(status){
   ...
   function adddkim(){
	var block_data='adddkim';  
	jQuery.ajax({
      url: "<?php echo base_url() ?>dashboard_extra/adddkim/",
      data:block_data,
      type:"POST",
      success: function(data) {
		var result = JSON.parse(data); 
			$(".success_msg").html("<p class='success_msg'>Thanks, message sent successfully to the support team.</p>");	
      }
    });
		...
		<div class="content-random">
          <h4>
            Add DKIM and SPF
              ...
              <a class="btn add"  href="javascript:void(0);" onclick="adddkim();"><i class="icon-envelope"></i>Email Us</a></a>
          </h4>
		  <div class="success_msg"></div>
          <div>
			
            <p>Click the button to request DKIM and SPF setup, our team will email you with instructions on how to setup the DNS records for these..   </p>
          </div>
		</div>
         <!--   Code added by jamshed 6/3/2018  -->
          ...
          <h4>
            Export GDPR Data
              ...
              <a class="btn add"  href="<?php echo base_url() ?>gdpr/exportcsv/"><i class="icon-envelope"></i>Export GDPR Data as a CSV File</a></a>
          </h4>
		  <div class="success_msg"></div>
          <div>
			
            <p>Click the button to download your all GDPR Data as a CSV File</p>
          </div>
		</div>
...
function exportcsv() {
      ...
      $.ajax({
        url:  'https://www.redcappi.com/gdpr/exportcsv/',
        success: function (data) {
