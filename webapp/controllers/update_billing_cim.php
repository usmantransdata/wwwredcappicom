	...
	*/
	function index(){
		...
		if($this->input->post('action')=='save'){
			...
			if($this->input->post('payment_type_name') != 'paypal'){
			...
			if($this->form_validation->run()==true){				
				...
				if ($this->input->post('payment_type_name') != 'paypal') {
					if($user_packages_array[0]['customer_payment_profile_id'] > 0){
						if($this->BillingModel->updateCustomerPaymentProfileRequest($user_packages_array[0]['customer_profile_id'],$user_packages_array[0]['customer_payment_profile_id'])){
							...
							if($payable_amount>0){
								...
								$update_array=array('member_payment_declined_count'=>0,'payment_type'=>'0');
								# update package id in memeber package table
								$this->UserModel->update_member_package($update_array,array('member_id'=>$user_packages_array[0]['member_id']));
							}
					...
					$update_array = array(
						...
						'paypal_payer_email' => $_POST['paypalEmail'],
						'payment_type'=>'1',
						'is_payment' => 1,
						'is_admin' => 0,
						'member_payment_declined_count' => 0
		...
		$user_package_detail = $this->UserModel->get_packages_data(array('package_id'=> $user_packages_array[0]['package_id']));
		//echo $this->db->last_query();exit;
		//echo '<pre>';print_r($user_package_detail);exit;
		$mode = $user_package_detail[0]['package_recurring_interval'];
		...
		$this->load->view('user/update_billing_cim',array('user_packages'=>$user_packages, 'arrUserForPaypal' => $arrUserForPaypal, 'user_package' => $user_packages_array[0],'country_info'=>$country_info,'messages'=>$messages,'mode'=>$mode,'credit_id'=> $this->CreditId));
