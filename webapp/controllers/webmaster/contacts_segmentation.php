...
class Contacts_segmentation extends CI_Controller
{
	...
	function srchcontactnow(){
		$e = $this->input->post('e');
		$rsContact = $this->db->query("select subscriber_id, member_username,if(subscriber_status = 1 and t1.is_deleted=0, 'active','inactive') c, status_change_date from red_email_subscribers t1 inner join red_members t2 on t1.subscriber_created_by=t2.member_id where subscriber_email_address like '%" . $e . "%'");
