	...
	*/
	...
	function acct_odd(){		
		...
		if($this->confg_arr['pmtalog_import_acct_odd'] == 'working'){
			...
			$source_dir = $this->config->item('pmta_logs');
                                                    $hold_dir = $this->config->item('pmta_logs_hold');
			$destination_dir = $this->config->item('pmta_archives');
			...
			foreach( $arrFiles as $f){	
                                                                    ...
                                                                    $filename = $f;
                                                                    $to_email = 'tech@redcappi.com';
                                                                    $from = 'tech@redcappi.com';
                                                                    $from_name = 'IT Tech';
                                                                    $subject = 'Working on File Process for Account File: ' . $filename;
                                                                    
                                                                    if ($intFilePrefixNo % 2 == 1){
                                                                        send_tmail($to_email, $from, $from_name, $subject, $subject, $subject);
                                                                    }
