...
class Cronjob extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('newsletter/Globallog_Model');
    ...
    function send($send = "") {
        ...
        if ($this->confg_arr['cronjob_status'] == "working") {
            ...
            //$fetch_conditions_array = array('campaign_id'=>221212, 'campaign_sheduled IS NOT NULL ' => NULL, 'is_deleted' => 0, 'campaign_status' => 'archived', 'is_segmentation' => 1);
            ...
            //$fetch_conditions_array = array('campaign_id'=>221212, 'campaign_sheduled IS NOT NULL ' => NULL, 'is_deleted' => 0, 'campaign_status' => 'archived');
            $campaign_count = $this->Campaign_Model->get_campaign_count($fetch_conditions_array);
            print_r("campaign count: " . $campaign_count);
            ...
            foreach ($campaigns as $campaign) {
                ...
                $this->Campaign_Model->update_campaign(array('email_send_date' => $date), array('campaign_id' => $campaign['campaign_id']));
                
                $last_digit_campaign = substr($campaign['campaign_id'],-1);
                
                //if ($last_digit_campaign % 2 == 1 ){
            
                 print_r("Working on campaign ID: " . $campaign['campaign_id']);
                ...
                $total_contact_selected = count($email_subscriber);
                print_r("total contacts to send: " . $total_contact_selected);
                ...
                if ($this->check_user_contacts($total_contact_selected, $campaign_created_by)) {
                    ...
                    if ($total_contact_selected > 0) {
                        ...
                        foreach ($email_subscriber as $subscriber_info) {
                            ...
                            if (!$this->is_authorized->ValidateAddress($subscriber_info['subscriber_email_address'])) {
                                ...
                                if ($mailCounter >= $campaign_batch_size) {
                                    print_r('send campaign in batch' . count($arrCampaigns));
                       ...
                       //  IF ARRAY SIZE IS LESS THAN 100
                        print_r('sent campaign in threads');
                        $this->send_campaign_in_threads($arrCampaigns);
                        unset($arrCampaigns);
                        $arrCampaigns = array();
                    ...
                    if ($campaign['is_segmentation'] == 1) {
                        // Archive campaign
                        $date = date("Y-m-d H:i:s");
                        print_r('updating campaign to active' . $campaign['campaign_id']);
                    ...
                    if (trim($is_seedlist) == '1') {
                        ...
                        if (count($arrCampaigns) > 0) {
                            print_r('sending capaign in threads' . $campaign['campaign_id']);
                            $this->send_campaign_in_threads($arrCampaigns);
                            unset($arrCampaigns);
                            $arrCampaigns = array();
                        }
        ...
        $totalRunningProcess = $this->checkProcessStatus();
        if ($this->confg_arr['cronjob_status_2'] == "working") {
            $this->alertIfStucked2();
            print_r('alert if stucked');
            //$this->move_email_queue_to_track();		
            exit;
        } elseif ($totalRunningProcess > 50) {
            // wait for 2 minutes and then reset the processes in red_campaign_thread
            print_r('total running processes > 60');
            $this->waitForProcessToComplete(120);
            //  before proceeding reset threads
            $this->db->query("update `red_campaign_thread` set `thread_status`='0' where 1");
            $this->ConfigurationModel->update_site_configuration(array('config_value' => 'completed'), array('config_name' => 'cronjob_status_2'));
            $utc_str = gmdate("Y-m-d H:i:s", time());
            $this->ConfigurationModel->update_site_configuration(array('config_value' => $utc_str), array('config_name' => 'campaign_cron_status_change_time_2'));
            exit;
        } else {
            ...
            // update cronjob status to completed
            $this->ConfigurationModel->update_site_configuration(array('config_value' => 'working'), array('config_name' => 'cronjob_status_2'));
            $utc_str = gmdate("Y-m-d H:i:s", time());
            $this->ConfigurationModel->update_site_configuration(array('config_value' => $utc_str), array('config_name' => 'campaign_cron_status_change_time_2'));
            ...
            foreach ($campaigns as $campaign) {
                ...
                $this->ConfigurationModel->update_site_configuration(array('config_value' => $thisCampaignId), array('config_name' => 'campaign_under_progress_2'));
                print_r('campaign in progress: ' .$campaign['campaign_id']);
                // Mark campaign_status "ready", if sender-name or sender-email or subject or campaign-content is empty 
                if (trim($campaign['email_subject']) == '' or trim($campaign['sender_name']) == '' or trim($campaign['sender_email']) == '' or trim($campaign['campaign_content']) == '') {
                    $this->db->query("update red_email_campaigns set campaign_status='ready' where campaign_id='$thisCampaignId'");
                    break;
                ...
                print_r("total contacts in array finalsub" . count($finalSub));
                ...
                if ($this->check_user_contacts($total_contact_selected, $campaign_created_by)) {
                    ...
                    if ($total_contact_selected > 0) {
                        ...
                        foreach ($email_subscriber as $subscriber_info) {
                            ...
                            if (!$this->is_authorized->ValidateAddress($subscriber_info['subscriber_email_address'])) {
                                ...
                                if ($mailCounter >= $campaign_batch_size) {
                                    print_r('sent campaign in batch with ' . $mailCounter . 'messages');
                                    $this->send_campaign_in_threads($arrCampaigns);
                                    unset($arrCampaigns);
                                    $arrCampaigns = array();
                                    $mailCounter = 0;
                    ...
                    if (trim($is_seedlist) == '1') {
                        ...
                        if (count($arrCampaigns) > 0) {
                            print_r('seedlist campaignt threads' . $campaign['campaign_id']);
                            $this->send_campaign_in_threads($arrCampaigns);
                            unset($arrCampaigns);
                            $arrCampaigns = array();
                        }
    ...
    function getFreeThreadID() {
        $query = $this->db->query("SELECT `thread_id` FROM `red_campaign_thread` where `thread_status` = '0' and `thread_id` < 150 limit 0,1 ");
...
function alertIfStucked2() {
        $last_sending_started_dt = $this->confg_arr['campaign_cron_status_change_time_2'];
        $timenow = gmdate("Y-m-d H:i:s", time());
        $timeDiffMinute = round(abs(strtotime($timenow) - strtotime($last_sending_started_dt)) / 60);
        if ($timeDiffMinute >= 30 && ($timeDiffMinute % 5) === 0) {
        //if ($timeDiffMinute >= 3) {
            $this->ConfigurationModel->update_site_configuration(array('config_value' => 'complete'), array('config_name' => 'cronjob_status_2'));
            $to = $this->confg_arr['admin_notification_email'];
            $msg = "Cron Process Updated -  Reset Cron Status to Complete";
            admin_notification_send_email($to, SYSTEM_EMAIL_FROM, 'RedCappi', "Cron was reset to Complete", $msg, $msg);
        }
