...
class Dashboard_extra extends CI_Controller
{
	...
	*/
	...
	function dashboard_extra_list(){
		...
		//   Code added by jamshed 6/3/2018   ////
		if($user_data_array[0]['gdpr_status']==0){
			$user_data['gdpr_status']=0;
			$user_data['gdpr_class']="inactive";
		}else{
			$user_data['gdpr_status']=1;
			$user_data['gdpr_class']="active";
		}
//   Code added by jamshed 6/3/2018   ////
...
$lang['disclaimer'] 		= "You are receiving this email because you signed up on our website or made a purchase from us. This is a commercial email and may contain a solicitation or advertisement.";
	...
	function adddkim(){
		...
		$member_email = $this->session->userdata('member_email_address');
		$support_email = 'support@redcappi.com';
		$mid =  $this->session->userdata('member_id');
		$vmta = $this->db->query('select vmta from red_members where member_id='.$mid)->row()->vmta;//'select vmta from red_members where member_id=11576';
		
		//$query = 'select * from red_member_from_email where member_id='.$this->session->userdata('member_id');
		$query = 'select * from red_member_from_email where is_verified=1 and member_id='.$mid;
		$finalArray = array();
		
		$rsGetEmails = $this->db->query($query);
		if($rsGetEmails->num_rows() > 0){
			foreach($rsGetEmails->result_array() as $key => $value){
				$finalArray[$key] = $value['email_address'];
			}
		}
		
		
		$message = "<p>Hi Support Team,</p><p>The user [$member_email] has requested SPF and DKIM to be set up for the sending addresses which are listed below: </p>";
	//	echo $message;exit;
		$i=1;
		if(count($finalArray)>0){
			...
			$message .= "<p>No other sending address available for this user</p>";
			
		}
		$message .= "The customer is also in the following pipeline: $vmta";
		
		$text_message = "SPF and DKIM to be set up: [$mid]";
		$this->load->helper('admin_notification');
		admin_notification_send_email($support_email, SYSTEM_EMAIL_FROM, 'RedCappi', "SPF and DKIM to be set up: [$mid]", $message, $text_message);
		//admin_notification_send_email('neha.citrusbug@gmail.com', SYSTEM_EMAIL_FROM, 'RedCappi', "SPF and DKIM to be set up: [$mid]", $message, $text_message);
