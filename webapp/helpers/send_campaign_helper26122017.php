...
function send_campaign_batch($campaign_file, $process_id) {
    ...
    foreach ($arrCampaignBatch as $eachBatch) {
        ...
        foreach ($eachBatch as $eachRecipient) {
            ...
            $sqlCampaignCreatedBy = "select campaign_created_by from red_email_campaigns where campaign_id =" . $thisCampaignId;
             $rsCampaignCreatedBy = $CI->db->query($sqlCampaignCreatedBy);
            $campaignCreatedBy = $rsCampaignCreatedBy->row()->campaign_created_by;
            
            //progress recruitment separation of vmta for hotmail
             if ($campaignCreatedBy = 16712){
                 if (stripos($thisEmailId,"hotmail") > 0){
                     $eachRecipient['vmta'] = 'redrotate';
                 }
             }
            if ($isThisSent == 0) {
                ...
                $mailtype = "newsletter";
                $mail->AddCustomHeader('Feedback-ID:' . ' CampaignID' . $thisCampaignId . ':CustomerID' . $eachRecipient['sender'] . ':MailTypeID' . $mailtype . ':redcappi');

                if (in_array($eachRecipient['vmta'], config_item('lw_vmta'))) {
                    $mail->Host = "host.redcappi.net";
                    $mail->Hostname = "host.redcappi.net";
                    $mail->Port = 2525;
                    $mail->AddCustomHeader('Sender: ' . $eachRecipient['sender']);
                    //$mail->Sender = base64_encode($subscriber_id) . "@bounce.redcappi.net";
                    $mail->ReturnPath = base64_encode($subscriber_id) . "@bounce.redcappi.net";
                    ...
                    $mail->ReturnPath = base64_encode($subscriber_id) . "@bounce.rcmailsv.com";
                    ...
                    $mail->ReturnPath = base64_encode($subscriber_id) . "@bounce.mailsvrc.com";
                    ...
                    $mail->ReturnPath = base64_encode($subscriber_id) . "@bounce.mailsvrc2.com";
                    ...
                    $mail->AddCustomHeader('Sender: ' . $eachRecipient['sender']);
                     ...
                     $mail->ReturnPath = base64_encode($subscriber_id) . "@bounce.mailsvrc3.com";
                    ...
                    $mail->ReturnPath = base64_encode($subscriber_id) . "@bounce.mailsvrc4.com";
                    ...
                    $mail->ReturnPath = base64_encode($subscriber_id) . "@bounce.mailsvrc5.com";
                   ...
                   //$mail->Sender = base64_encode($subscriber_id) . "@bounce.mailsvrc6.com";
                    ...
                    $mail->ReturnPath = base64_encode($subscriber_id) . "@bounce.mailsvrc6.com";
                    ...
                    $mail->ReturnPath = base64_encode($subscriber_id) . "@bounce.redcappi.com";
                    ...
                    $mail->Port = 587;
                    $mail->AddCustomHeader('Sender: ' . $eachRecipient['sender']);
                    $mail->ReturnPath= base64_encode($subscriber_id) . "@spark.redcappi.com";
                    ...
                    $mail->ReturnPath = base64_encode($subscriber_id) . "@bounce.rcmailcorp.com";
                ...
                $mail->AddCustomHeader('Message-ID:' . strtotime('now') . $eachRecipient['sender'] .  '@bounce.' . $eachRecipient['vmta']);
                $mail->AddCustomHeader('Precedence:bulk');
                $mail->AddCustomHeader('Return-Path:' . base64_encode($eachRecipient['sender']) . "@bounce." . $eachRecipient['vmta']);
...
function send_autoresponder_batch($message, $text_message, $subject, $sender_name, $sender, $campaign_id, $subscriber_info = array(), $campaign_type = 'html', $vmta = 'redrotate3') {
    ...
    $mail->ClearCustomHeaders();
    $mail->IsHTML(false);
    $mail->AddCustomHeader('Message-ID:' . strtotime('now') . $sender .  '@bounce.' . $vmta);
